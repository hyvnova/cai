...
lazy_static! {
    static ref VOICE_BLOCK_RE: Regex = Regex::new(r"\`\`\`say\s?([\w\W]*?)\s?\`\`\`").unwrap();
}
...
/// Empty function to satisfy the compiler if the feature is not enabled
...
#[cfg(feature = "voice")]
/// TODO: Make sure piper it's installe by allowing to set up the installation path
pub fn parse_say_block(response: &str, shell: &mut Shell) {
    if VOICE_BLOCK_RE.is_match(&response) {
        ...
        for (i, block) in blocks.enumerate() {
            ...
            shell.execute(
                &format!(
                    "echo \"{0}\" | piper --model \"{1}{2}.onnx\" --config \"{1}{2}.json\" --output_file \"{3}\" -q",
                    text,
                    PIPER_VOICE_PATH,
                    VOICE,
                    output_file
                ...
                Some(15)
            ).expect(format!("Failed to execute command: {}", text).as_str());
